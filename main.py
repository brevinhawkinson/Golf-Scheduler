...
st.set_page_config(page_title="Employee Scheduler",
                   page_icon="📅",
                   layout="wide")
...
class Employee:
    ...
    def __init__(self, name, unavailable_dates=None, mandatory_dates=None):
        ...
        self.mandatory_dates = mandatory_dates if mandatory_dates else [
        ]  # [(date, shift), ...]
...
def create_schedule(employees,
    ...
    mandatory_assignments = defaultdict(
        list)  # {(day_num, shift): [employee1, employee2, ...]}
...
def display_schedule(schedule, month, year, week=None):
    ...
    if week is not None:
        ...
        if week_days:
            ...
            st.subheader(
                f"📅 Generated Schedule for Week {week+1} ({start_day}-{end_day} {month_name} {year})"
            )
                ...
                if day == 0:  # Day belongs to previous/next month
                    ...
                    if day in schedule:
                        for shift, employees in schedule[day].items():
                            employee_str = ", ".join(employees) if isinstance(
                                employees, list) else employees
                            st.markdown(
                                f"*{shift}:* {employee_str}",
                                help=
    ...
    for day in sorted(schedule.keys()):
        ...
        for shift, employees in schedule[day].items():
            employee_names = ', '.join(employees) if isinstance(
                employees, list) else employees
            df_data.append({
                "Day": f"{day} ({day_name})",
                "Shift": shift,
...
def main():
    ...
    st.write(
        "Generate work schedules that respect employee availability restrictions"
    )
    ...
    with st.sidebar:
        ...
        selected_week_option = st.selectbox("Select Week",
        ...
        shifts_text = st.text_area("Enter shifts (one per line):",
                                   "Morning\nAfternoon\nEvening")
        shifts_per_day = [
            shift.strip() for shift in shifts_text.split('\n')
            if shift.strip()
        ]
    ...
    if 'mandatory_dates' not in st.session_state:
        st.session_state.mandatory_dates = {
        }  # {employee: [(date, shift), ...]}
    ...
    if new_employee_name:
        ...
        with col1:
            selected_date = st.date_input("Select date employee CANNOT work:",
        ...
        if st.button("Add Unavailable Date"):
            for shift in selected_shifts:
                ...
                if new_entry not in st.session_state.temp_employee[
                        "unavailable_dates"]:
                    st.session_state.temp_employee["unavailable_dates"].append(
                        new_entry)

        # Display current unavailable dates
        if st.session_state.temp_employee["unavailable_dates"]:
            st.write("Current unavailable dates:")
            for date_obj, shift in sorted(
                    st.session_state.temp_employee["unavailable_dates"]):
                st.write(
                    f"- {date_obj.strftime('%Y-%m-%d')} ({calendar.day_name[date_obj.weekday()]}) - {shift}"
                )
        ...
        col1, col2 = st.columns(2)
        with col1:
            mand_date = st.date_input("Select date employee MUST work:",
                                      value=date(year, month, 1),
                                      min_value=date(year, month, 1),
                                      max_value=date(
                                          year, month,
                                      ...
                                      key="mand_date")
        with col2:
            mand_shifts = st.multiselect("Select shifts employee must work:",
                                         options=shifts_per_day,
                                         key="mand_shifts")
        ...
        if st.button("Add Mandatory Date"):
            for shift in mand_shifts:
                ...
                if new_entry not in st.session_state.temp_employee[
                        "mandatory_dates"]:
                    st.session_state.temp_employee["mandatory_dates"].append(
                        new_entry)
        ...
        if st.session_state.temp_employee["mandatory_dates"]:
            ...
            for date_obj, shift in sorted(
                    st.session_state.temp_employee["mandatory_dates"]):
                st.write(
                    f"- {date_obj.strftime('%Y-%m-%d')} ({calendar.day_name[date_obj.weekday()]}) - {shift}"
                )
        ...
        if st.button("Add Employee"):
            if new_employee_name:
                ...
                employee.unavailable_dates = st.session_state.temp_employee[
                    "unavailable_dates"]
                employee.mandatory_dates = st.session_state.temp_employee[
                    "mandatory_dates"]

                if not any(e.name == new_employee_name
                           for e in st.session_state.employees):
                    if 'employees' not in st.session_state:
                        st.session_state.employees = []
                    st.session_state.employees.append(employee)
                    st.success(
                        f"Added {new_employee_name} to the employee list.")
                    # Reset temp employee
                    st.session_state.temp_employee = {
                        "name": "",
                        "unavailable_dates": [],
    ...
    if st.session_state.employees:
        ...
        for emp in st.session_state.employees:
            ...
            for date_obj, shift in emp.unavailable_dates:
                unavailable_dates_str.append(
                    f"{date_obj.strftime('%Y-%m-%d')} ({shift})")

            # Format mandatory dates
            mandatory_dates_str = []
            for date_obj, shift in emp.mandatory_dates:
                mandatory_dates_str.append(
                    f"{date_obj.strftime('%Y-%m-%d')} ({shift})")
            ...
            employee_data.append({
                ...
                emp.name,
                "Cannot Work On":
                ", ".join(unavailable_dates_str)
                if unavailable_dates_str else "None",
                "Must Work On":
                ", ".join(mandatory_dates_str)
                if mandatory_dates_str else "None"
            })
    ...
    if st.session_state.employees and shifts_per_day:
        if st.button("Generate Schedule"):
            if len(st.session_state.employees) < people_per_shift:
                st.error(
                    f"Not enough employees! You need at least {people_per_shift} employees to meet the requirement of {people_per_shift} people per shift."
                )
            else:
                with st.spinner("Generating schedule..."):
                    ...
                    for emp in st.session_state.employees:
                        ...
                        new_emp = Employee(
                            emp.name, emp.unavailable_dates.copy(),
                            emp.mandatory_dates.copy() if hasattr(
                                emp, 'mandatory_dates') else [])
                        employees_copy.append(new_emp)

                    # Create schedule with the selected week (if any)
                    schedule = create_schedule(employees_copy, shifts_per_day,
                                               people_per_shift, month, year,
                                               selected_week)
    ...
    if 'schedule' in st.session_state:
        display_schedule(
            ...
            st.session_state.get(
                'schedule_week')  # Pass the selected week if it exists
        )

        # Option to download the schedule as CSV
        schedule_data = []
        for day in sorted(st.session_state.schedule.keys()):
            day_name = calendar.day_name[calendar.weekday(
                st.session_state.schedule_year,
                st.session_state.schedule_month, day)]
        ...
        if schedule_data:
            ...
            st.download_button(
                ...
                data=csv,
                file_name=
                f"employee_schedule_{month_name}_{st.session_state.schedule_year}.csv",
                mime='text/csv',
            )
        ...
        if hasattr(
                st.session_state,
                'shift_distribution') and st.session_state.shift_distribution:
            st.write("### 📊 Shift Distribution Analysis")
            st.write(
                "This analysis shows how shifts are distributed among employees:"
            )
            ...
            if shift_data:
                # Sort by shift count
                shift_data.sort(key=lambda x: x["Number of Shifts"],
                                reverse=True)
                ...
                avg_shifts = sum(d["Number of Shifts"]
                                 for d in shift_data) / len(shift_data)
                ...
                if max_shifts - min_shifts <= 1:
                    st.success(
                        "✅ Excellent balance! All employees have a similar number of shifts."
                    )
                elif max_shifts - min_shifts <= 2:
                    st.info(
                        "ℹ️ Good balance. The difference between most and least shifts is 2 or less."
                    )
                else:
                    st.warning(
                        f"⚠️ There is some imbalance. The difference between most and least shifts is {max_shifts - min_shifts}."
                    )
